...
> 日期：2026-07-31（更新数据快照与 Phase 状态）
>
> 上次审计：2026-07-27
>
...
| 对象 | 数量 | 说明 |
|---|---:|---|
| Users | 45 | 已有真实登录与角色数据 |
| Spaces | 2,078 | 其中 active 2,064 |
| Guides | 2,063 | 全部已发布（2,063） |
| Chat messages | 50 | 实时聊天已产生持久化数据 |
| Space traces | 12 | 地点留言/痕迹已投入使用 |
| Space capsules | 7 | 双锁打开与双现场证明写入数据 |
| Helps | 0 | 只有表结构，没有产品模块 |
| Games | 0 | 只有表结构，没有产品模块 |
| Space members | 0 | 只有表结构，成员系统未产品化 |
| Space templates | 0 | 只有表结构，模板数据闭环未落地 |
| Site page versions | 8 | 首页 CMS 草稿/发布版本已工作 |
| Agent API keys | 0 | 功能已上线；当前无活动 Key（临时测试 Key 已清理） |
| Agent API audit log | 1 | 历史测试审计残留 |
...
|---|---|---|---|
| Phase 1 用户主闭环 | 已实现 | 注册、登录、退出、全局会话刷新、创建空间、地图出现、我的空间、导航与中英切换 | 创建后跨页面刷新仍需持续回归；错误反馈缺统一模型（待做） |
...
| Phase 4 攻略系统 | 已实现 | 分类分页、详情、结构化编辑、绑定/解绑空间、更新同一攻略、删除/归档、用户与管理员权限；**服务端多关键词搜索、国家→省→市→区→地点真实级联筛选**（2026-07-30）；**媒体上传端点 `/api/media/upload` + 本地 `/uploads` 资产服务**；**每次创建/编辑自动写 `guide_versions` 快照，编辑页可回退任意版本**（2026-07-31） | 协作修订与 AI 辅助整理未做（属长期项）；上传存本地磁盘，未接对象存储 |
| Phase 5 后台运营 | 已实现 | 概览、首页编辑器、空间、攻略、驻留、用户角色、审计日志、主标题显式断行；**攻略后台服务端分页/搜索/状态筛选 + 总数**；**空间/攻略 CSV 导出（管理员）**；**危险操作两步确认（删除攻略、删除空间等）**（2026-07-31） | 完整运营工作流（批量、通知、报表）属长期项 |
| Phase 6 实时互动 | 已实现基础 | 独立聊天页、WebSocket 广播、历史持久化、在线人数、私密版本校验、重连；**消息类型 text/system/help/help_resolved**；**求助最小闭环（helps 表 + 聊天页发布/解决）**；**房间连接上限 300 + 每连接 400ms 消息限流**（2026-07-31） | 通知、送达状态、成员列表实时化、游戏未实现（长期项） |
| Phase 7 数据迁移与生产化 | 已实现 | 启动迁移、健康/就绪检查、空间过期任务、部署脚本、测试集合、GeoNames/种子导入；**迁移漂移修复（admin_audit_log）、WASM/SSR 版本同步部署（v91→v92）**；**importer 支持 `--import --pg` 真实导入（幂等防重）**；**CI（GitHub Actions：fmt + server check + wasm hydrate check + 单测）**；**启动时 schema contract 校验（漂移日志报错）**；**备份/恢复演练文档 `docs/BACKUP_AND_RECOVERY.md`**（2026-07-31） | 对象存储、多活部署属长期项 |

> 2026-07-30 额外闭环：攻略行“阅读”从无链接 span 改为真实 `<a>`；首页全站搜索 `q` 参数传递到探索页；Agent REST API 上线（`/api/spaces`、`/api/guides` 的列表/创建/更新，2026-07-31 补详情 GET 与 DELETE）——见白皮书 Phase 3 缺口追踪。
...
**当前状态：早期基础（2026-07-30/31 已落地 Agent REST API）。**

已经具备：

- 独立 Axum REST（不暴露 Leptos Server Functions）：`/api/spaces`、`/api/guides`
- API Key 鉴权（Bearer / `X-Inspace-Api-Key`）、scope 读写分离、60 次/分钟限流、全量审计日志
- 空间与攻略的列表/创建/详情/更新/删除；教程见 `docs/AGENT_API_TUTORIAL.md`

尚未完成：

- 稳定的公开 Open API（面向第三方开发者）；
...
### 2026-07-30/31 已落地（对照上文缺口）

1. ✅ 攻略列表“阅读”修复为真实链接；探索/攻略页服务端分页 + 国家→省→市→区→地点真实级联筛选（基于当前可发现数据，避免空结果选项）。
2. ✅ 首页全站搜索 `q` 传递到探索页并保留在输入框。
3. ✅ WASM/SSR 版本同步：v91 同步部署，移除 nginx 过期 `sub_filter`；hydration 恢复（菜单/阅读均正常，Playwright 验证无 console error）。
4. ✅ Agent REST API 上线并补齐：`/api/spaces`、`/api/guides` 列表/创建/PATCH/GET 详情/DELETE，独立 Axum REST + API Key scope 鉴权 + 限流 + 审计；临时 Key/数据全部清理。
