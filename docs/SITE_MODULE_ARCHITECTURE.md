...
| Phase 5 后台运营 | 部分实现 | 概览、首页编辑器、空间、攻略、驻留、用户角色 | 审计表生产漂移；危险操作确认不统一；缺服务端分页/导出；首页编辑器主标题不支持明确手动断行；后台仍缺运营工作流 |
...
- DB：`crates/db/src/traces.rs`
- Domain：`CapsuleSummary`、`CapsuleOpenResult`
- 表：`space_capsules`
- 典型 Bug：在地点仍打不开、口令校验顺序错、失败次数错误、已打开状态不更新。
- 核心规则：必须同时满足“现场 Wi-Fi 口令”和“胶囊作者口令”；GPS/QR 不能替代现场口令。
